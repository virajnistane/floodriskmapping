...
from src.coastline_buffer import (
    CoastlineBuffer,
    create_coast_buffer,
...
def test_create_coast_buffer_returns_geodataframe(dem_dataset):
    ...
    buffered = create_coast_buffer(coast_gdf, buffer_dist_m=1000.0, dem_crs=dem_dataset.crs)
...
def test_rasterize_coast_buffer_returns_boolean_array(dem_dataset):
    ...
    geom = box(dem_dataset.bounds.left, dem_dataset.bounds.bottom, 
               dem_dataset.bounds.left + 100, dem_dataset.bounds.bottom + 100)
...
def test_rasterize_coast_buffer_has_true_values(dem_dataset):
    ...
    geom = box(center_lon - size/2, center_lat - size/2,
...
def test_coastline_buffer_different_buffer_distances(coastline_path):
    ...
    assert mask_10km.sum() >= mask_5km.sum(), \
...
def test_create_coast_buffer_with_zero_distance(dem_dataset):
    ...
    buffered = create_coast_buffer(coast_gdf, buffer_dist_m=0.0, dem_crs=dem_dataset.crs)
